...
test("presents the human and AI WebMCP experiences on the landing page", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    page.getByRole("heading", {
      name: "Your household finances, ready for you and your AI.",
    }),
  ).toBeVisible();
  await expect(page.getByRole("link", { name: "Try the demo" })).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Explore dashboard" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: "A simple handoff between you, your AI, and Koshara.",
    }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: "See how your AI can work with Koshara",
    }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Understand your finances" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Import a statement" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Try the statement workflow" }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Copy prompt" })).toBeVisible();
  await expect(page.getByRole("button", { name: "View prompt" })).toBeVisible();
  await expect(
    page.getByText(/Import the attached demo statement into Koshara/),
  ).not.toBeVisible();
  ...
  await expect(
    page.getByRole("heading", {
      name: "A dashboard for you. Structured capabilities for your AI.",
    }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("heading", { name: "For your AI", exact: true }),
  ).toBeVisible();
});

test("serves the synthetic sample statement as a PDF", async ({ request }) => {
  const response = await request.get(
    "/koshara_demo_credit_card_statement_june_2026.pdf",
  );
...
test("aligns the landing navigation with the page content", async ({
  page,
}) => {
  await page.goto("/");

  const navigationBounds = await page
    .getByRole("navigation", { name: "Koshara navigation" })
    .boundingBox();
  ...
  expect(
    Math.abs(navigationBounds!.x - heroContainerBounds!.x),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.abs(navigationBounds!.width - heroContainerBounds!.width),
  ).toBeLessThanOrEqual(1);
  expect(
    Math.abs(
      brandBounds!.x - (heroContainerBounds!.x + heroPaddingInlineStart),
    ),
  ).toBeLessThanOrEqual(1);
});
