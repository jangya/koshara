...
describe("date range presets", () => {
  ...
  it("uses the elapsed local calendar month for this month", () => {
    expect(getDateRangePreset("this-month", now)).toEqual({
      start: "2026-08-01",
      end: "2026-08-30",
    });
  });
  ...
  it("returns complete calendar boundaries for previous month presets", () => {
    expect(getDateRangePreset("last-month", now)).toEqual({
      start: "2026-07-01",
      end: "2026-07-31",
    });
    expect(getDateRangePreset("last-3-months", now)).toEqual({
      start: "2026-06-01",
      end: "2026-08-30",
    });
    expect(getDateRangePreset("last-6-months", now)).toEqual({
      start: "2026-03-01",
      end: "2026-08-30",
    });
    expect(getDateRangePreset("year-to-date", now)).toEqual({
      start: "2026-01-01",
      end: "2026-08-30",
    });
  });
  ...
  it("compares against the immediately preceding inclusive period of equal length", () => {
    expect(
      getPreviousPeriod({ start: "2026-08-01", end: "2026-08-30" }),
    ).toEqual({
      start: "2026-07-02",
      end: "2026-07-31",
    });
  ...
  it("accepts valid URL ranges and falls back to the last-month default when values are incomplete or reversed", () => {
    expect(
      parseDateRangeParams(
        new URLSearchParams("from=2026-07-04&to=2026-08-09"),
        now,
      ),
    ).toEqual({
      preset: "custom",
    ...
    expect(parseDateRangeParams(new URLSearchParams(""), now)).toEqual({
      preset: "last-month",
      range: { start: "2026-07-01", end: "2026-07-31" },
    });
  });
  ...
  it("formats exact periods without ambiguous preset-only text", () => {
    expect(formatDateRange({ start: "2026-08-01", end: "2026-08-31" })).toBe(
      "August 2026",
    );
    expect(formatDateRange({ start: "2026-08-01", end: "2026-08-30" })).toBe(
      "1–30 August 2026",
    );
    expect(formatDateRange({ start: "2025-12-20", end: "2026-01-04" })).toBe(
...
describe("timeline aggregation", () => {
  ...
  it("groups short ranges daily and fills dates without activity", () => {
    expect(
      aggregateTimeline(transactions, {
        start: "2026-08-01",
        end: "2026-08-03",
      }),
    ).toEqual([
  ...
  it("groups ranges longer than 93 days by calendar month", () => {
    const points = aggregateTimeline(transactions, {
      start: "2026-05-01",
      end: "2026-09-30",
    });
